...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $building = trim($_POST['building_num']); 
    ...
    $updateSql = "UPDATE users SET building = ? WHERE username = ?";
    $updateStmt = $conn->prepare($updateSql);
    if ($updateStmt) {
        $updateStmt->bind_param('ss', $building, $username);
        if ($updateStmt->execute()) {
            $message = "Building updated successfully!";
        } else {
            $message = "Error updating building: " . $updateStmt->error;
        }
        $updateStmt->close();
    } else {
...
<head>
    ...
    <title>Change Parking building</title>
    ...
    <main>
        <div class="login-form-container">
            <div id="multi-step-form" class="login-form">
                <h2><strong>Change Parking building</strong></h2>
                ...
                <form action="update_building.php" method="POST">
                    <div class="input-box">
                        ...
                        <select class="action-list" name="building_num" required>
                            <option value="17"><strong>Pakring Building 17</strong></option>
                            <option value="4"><strong>Pakring Building 4</strong></option>
                        </select>
                    </div>
                    <div class="input-box">
                        <button class="action-btn" type="submit"><strong>Change</strong></button>
                    </div>
                </form>
            </div>
        </div>
